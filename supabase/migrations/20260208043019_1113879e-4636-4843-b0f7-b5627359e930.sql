-- Update rpc_get_agent_tasks to include tenant_domain as organization in PowerShell tasks
...
BEGIN
  ...
  FROM (
    ...
    -- Gera step dinâmico a partir do payload
    -- UPDATED: Now includes tenant_domain as organization
    -- ==========================================
    SELECT
      t.id,
      t.task_type,
      ...
      -- Gerar step dinâmico a partir do payload (usando jsonb para consistência)
      -- UPDATED: organization now uses tenant_domain (e.g., "company.onmicrosoft.com")
      jsonb_build_object(
