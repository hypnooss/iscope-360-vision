  ...
  return (
    <div className="group">
      <div className="flex items-start gap-3 py-2">
        <span className="text-xs text-muted-foreground w-28 flex-shrink-0 uppercase tracking-wide pt-0.5">
          {label}
        </span>
...
export default function FirewallReportPreview() {
  const { hostname, score, vendor, model, serial, firmware, uptime, stats, security } = mockData;

  return (
    <div className="min-h-screen bg-background p-6">
      {/* Page Title */}
      <div className="mb-8">
        <h1 className="text-2xl font-bold text-foreground">Firewall Report Header</h1>
        <p className="text-muted-foreground text-sm mt-1">
          Preview do header de relatório para análise de Firewall
        </p>
      </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12">
              ...
              <div className="flex flex-col justify-center lg:border-l lg:border-border/30 lg:pl-8">
                ...
                <DetailRow 
                  ...
                  value={security.adminPasswordChanged ? "Alterada" : "Padrão"} 
                  indicator={security.adminPasswordChanged ? "success" : "error"}
                />
                <DetailRow 
                  label="HA Cluster" 
                  value={security.haEnabled ? "Ativo" : "Inativo"} 
                  indicator={security.haEnabled ? "success" : "error"}
                />
                <DetailRow 
                  label="Syslog" 
                  value={security.syslogConfigured ? "Configurado" : "Ausente"} 
                  indicator={security.syslogConfigured ? "success" : "error"}
                />
              </div>
            </div>
          </div>
      ...
      <div className="max-w-5xl mt-8 glass-card rounded-xl p-6 border border-border/50">
        <h2 className="text-lg font-semibold mb-4">Características do Design - Firewall</h2>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-sm text-muted-foreground">
          <ul className="space-y-2">
            <li>✓ Hostname em caixa alta com tracking-[0.2em]</li>
            <li>✓ Gauge 180px com glow radial sutil</li>
            <li>✓ MiniStats com cores diferenciadas</li>
            <li>✓ Informações técnicas: Vendor, Model, Serial</li>
          </ul>
          <ul className="space-y-2">
            <li>✓ Firmware e Uptime do dispositivo</li>
            <li>✓ Indicadores de segurança: Admin Password, HA, Syslog</li>
