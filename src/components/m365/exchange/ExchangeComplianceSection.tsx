...
interface ExchangeComplianceSectionProps {
  category: string;
  label: string;
...
export function ExchangeComplianceSection({ category, label, items, index }: ExchangeComplianceSectionProps) {
      ...
      >
        <div className="flex items-center gap-3 flex-wrap">
          <div className="p-2 rounded-lg" style={{ backgroundColor: `${colorHex}15` }}>
            <DynamicIcon name={iconName} className="w-5 h-5" style={{ color: colorHex }} />
          </div>
      ...
      {isExpanded && (
        ...
        >
          {failedItems.map((item) => (
            <UnifiedComplianceCard key={item.id} item={item} />
          ))}
